...
/* ----------------------------- TMDB Endpoints ----------------------------- */ function endpointForSection(section, page = 1) { if (section === 'trending') return ${API_BASE}/trending/movie/week?page=${page}; if (section === 'now_playing') return ${API_BASE}/movie/now_playing?page=${page}; if (section === 'top_rated') return ${API_BASE}/movie/top_rated?page=${page}; return ${API_BASE}/movie/popular?page=${page}; }
...
/* ----------------------------- GiftedTech Helpers ----------------------------- */ async function giftedSearch(title) { try { const url = ${GIFTED_BASE}/search?query=${encodeURIComponent(title)}; const res = await fetch(url); if (!res.ok) return null; const data = await res.json(); if (!data.results || data.results.length === 0) return null; return data.results[0].id; } catch (e) { return null; } }

async function giftedSources(id) { try { const res = await fetch(${GIFTED_BASE}/sources/${id}); if (!res.ok) return { items: [], subtitles: [] }; const data = await res.json(); return { items: data.results ?? data.sources ?? data.items ?? [], subtitles: data.subtitles ?? data.subs ?? [] }; } catch (e) { return { items: [], subtitles: [] }; } }

/* ----------------------------- Movie Modal (TMDB + GiftedTech) ----------------------------- */ async function openModal(movieId) { try { modal.classList.remove('hidden'); document.body.style.overflow = 'hidden';
...
/* ----------------------------- GiftedTech Download + Streaming ----------------------------- */ async function loadDownloadLinks(title) { try { modalDownload.innerHTML = '<p class="muted">Loading download links...</p>';
...
/* ----------------------------- Utilities ----------------------------- */ function formatBytes(bytes) { if (!bytes || isNaN(bytes)) return String(bytes); const kb = 1024; if (bytes < kb) return bytes + ' B'; if (bytes < kb * kb) return (bytes / kb).toFixed(1) + ' KB'; if (bytes < kb * kb * kb) return (bytes / (kb * kb)).toFixed(1) + ' MB'; return (bytes / (kb * kb * kb)).toFixed(1) + ' GB'; }
